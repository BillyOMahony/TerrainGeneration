...
float PerlinAlgo::Fade(float t)
{
	return t * t * t * (t * (t * 6 - 15) + 10);
}

float PerlinAlgo::Grad(int32 hash, float x, float y, float z)
{
	hash &= 15;
	float u = (hash < 8) ? x : y;
...
float PerlinAlgo::Lerp(float x, float a, float b)
{
	return a + x * (b - a);
}

float PerlinAlgo::Noise(float X, float Y, float Z)
{
	TArray<int32> P = {
